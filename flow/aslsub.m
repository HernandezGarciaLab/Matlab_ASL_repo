...
if order==0
    %order = 0 means that odds are tags, evens are controls 
	tmp = t;
	t = c;
	c = tmp;
end
...
% 	write_img('var_magDiff.img', vmd  , h);
	spd = std(pd,0,1);
	write_img('std_phaseDiff.img', spd * 1e6 , h);

	smd = std(md,0,1);
	write_img('std_magDiff.img', smd  , h);
